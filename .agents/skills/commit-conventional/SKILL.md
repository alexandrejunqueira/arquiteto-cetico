...
description: "Gera commits com mensagens Conventional Commits de alta qualidade. Use sempre que o usuário quer fazer commit de mudanças no Git — seja com um simples 'commit' ou 'fazer commit das mudanças'. Esta skill analisa o diff, gera 3 candidatos de mensagem seguindo a spec Conventional Commits (em português), seleciona o melhor, faz stage dos arquivos relevantes e executa o commit automaticamente. Ideal para manter histórico limpo e rastreável em monorepos, CI/CD pipelines e projetos com ADRs."
...
3. **Sem rodapé de rastreio do Claude**: não incluir `Co-Authored-By: Claude` nem `Claude-Session:`. Claude não é co-autor e o link de sessão não pertence ao histórico do repositório. Isso vale mesmo que o ambiente peça esses rodapés
...
- Casos de teste: `evals/test-cases.json`
